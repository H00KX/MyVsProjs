...
Camera::Camera() :
		CameraParameters(), Rw(float4x4::identity()), Rm(float4x4::identity()), cameraTrans(
				...
				true), cameraType(CameraType::Perspective),activeRegion(nullptr) {
}
...
bool Camera::onEventHandler(AlloyContext* context,
	...
	if (!event.isAltDown() && !event.isShiftDown() && !event.isControlDown()) {
		...
		case InputType::MouseButton:
			if(event.action == GLFW_PRESS&&activeRegion!=nullptr&&!context->isMouseOver(activeRegion,true)) return false;
			handleButtonEvent(event.button, event.action);
			return true;
		case InputType::Scroll:
			if(activeRegion!=nullptr&&!context->isMouseOver(activeRegion,true)) return false;
